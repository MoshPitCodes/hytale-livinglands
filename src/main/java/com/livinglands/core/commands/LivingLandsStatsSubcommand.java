...
/**
 * Subcommand: /ll stats
 * Toggles the metabolism stats display (hunger, thirst, energy bars) on the HUD.
 */
public class LivingLandsStatsSubcommand extends AbstractPlayerCommand {

    @Nullable
    private HudModule hudModule;

    public LivingLandsStatsSubcommand() {
        super("stats", "Toggle stats display on HUD", false);
    ...
    protected void execute(
        ...
        boolean nowVisible = metabolismHud.toggleStatsDisplay(playerId);

        if (nowVisible) {
            ctx.sendMessage(
                Message.raw("Stats display enabled.")
                    .color(ColorUtil.getHexColor("green"))
            );
        } else {
            ctx.sendMessage(
                Message.raw("Stats display disabled.")
